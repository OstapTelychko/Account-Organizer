    ...
    for category in Session.account.get_all_categories():
        ...
        if category[1] == "Incomes":
            for transaction in Session.account.get_all_transactions(category[0]):
                Session.current_total_income = Session.current_total_income + Decimal(transaction[5])

        elif category[1] == "Expenses":
            for transaction in Session.account.get_all_transactions(category[0]):
                Session.current_total_expenses = Session.current_total_expenses + Decimal(transaction[5])
    
    Session.current_total_income = float(Session.current_total_income)
    Session.current_total_expenses = float(Session.current_total_expenses)
