...
class Account():
    ...
    def account_exists(self)-> bool:
        with self.connection:
            result = self.cursor.execute("select * from 'Accounts' where account_name=?",(self.user_name,)).fetchone()
            return bool(result)
    ...
    def get_account_id(self)-> int:
        with self.connection:
            id = self.cursor.execute("select id from 'Accounts' where account_name=?",(self.user_name,)).fetchone()[0]
            return(id)


    def get_account_balance(self)-> int|float:
        with self.connection:
            current_balance = self.cursor.execute("select current_balance from 'Accounts' where account_name=?",(self.user_name,)).fetchone()[0]
            return current_balance


    def update_account_balance(self,balance:float|int):
        with self.connection:
            self.cursor.execute("update 'Accounts' set balance=?, where account_name=?",(balance,self.user_name,))


    def rename_account(self,account_id:int,new_user_name:str):
        with self.connection:
            self.cursor.execute("update 'Accounts' set account_name=? where id = ?",(new_user_name,account_id,))
            self.user_name = new_user_name
    

    def delete_account(self):
        with self.connection:
            self.cursor.execute("delete  from 'Accounts' where account_name=?",(self.user_name,))
    ...
    def create_category(self,catagory_name:str,category_type:str):
        with self.connection:
            self.cursor.execute("insert into 'Categories' (type,category_name,account_id) values (?,?,?)",(category_type,catagory_name,self.account_id,))
